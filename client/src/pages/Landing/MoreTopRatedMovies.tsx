...
const MoreTopRatedMovies = () => {
    const [loading, setLoading] = useState<boolean>(false);
    const [topMovies, setTopMovies] = useState<IMoviesByGenre[]>([]);
    ...
    useEffect(() => {
        const handleGetTrendingMovies = async () => {
            try {
                ...
                const response = await axios.get(`https://api.themoviedb.org/3/movie/top_rated?page=${currentPage}&api_key=${apiKey}`, config);
...
export default MoreTopRatedMovies;