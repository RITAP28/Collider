...
const MovieRecommendationSection = ({
  ...
  const handleMovieRecommendations = async () => {
    ...
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/${movieDetails.id}/recommendations?language=en-US&page=1&api_key=${apiKey}`, config);
  ...
  return loading
    ...
    : recommendations.map((movie, index) => (
        <div className="min-w-[10rem] flex flex-col" key={index}>
          ...
          <div className="w-full font-Manrope font-semibold pl-2 text-lg py-2">
